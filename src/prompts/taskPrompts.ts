...
2. 議事録内で、既存のタスク一覧に該当しない、新しく決定されたタスク（Todo）がある場合：
   ...
   - 既存のタスク一覧の中で、この新規タスクが依存している（このタスクの前提となる、または関連して次に進めるべき）タスクがある場合、該当するタスクのIDを配列で dependencies に含めてください。関連がない場合は空配列 [] にしてください。
...
[
  {
    ...
    "dependencies": ["依存・関連する既存タスクのID（なければ空配列）"]
  },
  {
    "type": "update",
    "id": "既存タスクのTask ID",
